...
### 3.3 `assembleTrack` — التقرير **يقرأ** ولا يطابق (لكل طالب/مسار/يوم — §13)

**المطابقة تحدث مرّة واحدة، في وحدة الإنجازات، وتُخزَّن.** `PlanReconciliationService.reconcilePlan` يسوّي الأسبوع ويكتب جدول `achievement_plan_item_links`: صف لكل مقطع، يحمل `weekly_plan_item_id` (أو NULL لِما هو خارج الخطة)، `achievement_id`، المدى بالفهرس العالمي، الآيات، الصفحات، ونتيجة الإنجاز الفائز. التسوية نفسها (`achievements/logic/settlement.ts`):

1. عناصر الخطة مرتّبة بالأولوية (اليوم ثم `order` ثم `id`) تستهلك من مجمّع آيات الأسبوع بالتتابع.
2. المقطع المستهلَك يُقسَّم على حدود الإنجازات → **مقاطع ذرّية**؛ كل مقطع يُنسب لأعلى `percentage_score`، وعند التعادل الأحدث `approved_at` ثم أعلى `id` (حتمية).
3. المقاطع المتجاورة بنفس الفائز تُدمج. وما غطّاه الإنجاز ولم يخطّطه أي بند → صفوف `outside` بـ `weekly_plan_item_id = NULL`.
   - **نطاق الإنجاز المحتسَب = مدى الإنجاز كاملاً، بما في ذلك `recitation_method='test'`.** الاختبار في مواضع مختارة أسلوبٌ لاختصار الوقت في المراجعة القريبة/البعيدة، لا تقصيرٌ في الخطة؛ فأخطاء تلك المواضع تنعكس في `percentage_score` (الجودة) فقط ولا تنقص التغطية.

**أي تعديل على الخطة يعيد بناء روابط الأسبوع كاملةً** (حذف ثم إدراج لكل `reconcilePlan`)، فلا يبقى إنجاز مرتبطاً بنطاق قديم لبند نُقل أو عُدِّل.

`assembleTrack(trackType, plannedItems, links)` في التقرير لا يطابق نطاقات إطلاقاً — يجمّع فقط:
- `plannedPages` من اتحاد بنود اليوم، `achievedPages` من اتحاد مدَيات الروابط.
- `completion_rate = min(achieved/planned×100, 100)` (§14.3)، `quality_rate` = متوسط النتائج موزوناً بتغطية صفحات كل رابط (§15.4).
- `gaps` = بنود اليوم ناقص الروابط. `outsidePlanSegments` = روابط الـ NULL بتاريخ إنجاز = تاريخ التقرير.
- **النطاق أسبوعي:** بند يوم الاثنين قد يُسوّى بإنجاز يوم الأربعاء من نفس الأسبوع (قرار المالك)، ويظهر ذلك في `achievementDate` على كل مقطع.
- **الإخراج:** `{version, trackType, plannedPages, achievedPages, completionRate, qualityRate, plannedRanges[], approvedSegments[], gaps[], outsidePlanSegments[]}` مطابقاً للبنية في §12، مع إضافة `planItemId` و`achievementDate` على كل `approvedSegment` (وحذف `candidateAchievementIds`).

### 3.4 إعادة توزيع الأوزان — `weight-redistribution.ts` (§6)
- المسارات الموجودة من **الخطة المعتمدة** لذلك اليوم (لا من الإنجاز).
- `academic_weight = 100 − ethics_weight`. الأخلاق لا يُعاد توزيعها. حالة "لا خطة" §6.3.
- **إعادة التوزيع تُراعي العائلة أولاً:** وزن المراجعة يبقى داخل عائلة المراجعة ما دام أحد نوعيها مخطّطاً:
  - قريب مخطّط والبعيد غير مخطّط → وزن البعيد **كاملاً** للقريب (والعكس صحيح). وزن الحفظ لا يتأثر.
  - لا قريب ولا بعيد → وزن المراجعتين معاً يذهب للحفظ.
  - الحفظ غير مخطّط → وزنه يُوزّع على مسارات المراجعة المخطّطة **توزيعاً معيارياً** (بنسبة أوزانها الأساسية؛ وبالتساوي إن كانت أوزانها صفراً).
  - كل المسارات مخطّطة → `effective = base` بلا تغيير.
- المجموع النهائي يُقيَّس ليساوي `academic_weight` دائماً.
