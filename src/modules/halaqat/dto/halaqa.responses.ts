...
export class TeacherAssignmentResponse {
  ...
  @ApiProperty({ enum: ['main', 'assistant', 'substitute'], example: 'main' })
  role!: TeacherRole;
  @ApiProperty({ example: false }) acting_as_primary!: boolean;
  @ApiProperty({ nullable: true, example: null }) acting_starts_at!:
    | string
    | null;
  @ApiProperty({ nullable: true, example: null }) acting_ends_at!:
    | string
    | null;
...
export class HalaqaListItem {
  ...
  @ApiProperty({ enum: ['active', 'archived', 'completed'], example: 'active' })
  status!: HalaqaStatus;
  @ApiProperty({ nullable: true, type: PrimaryTeacherSummary })
  primary_teacher!: PrimaryTeacherSummary | null;
...
export class HalaqaDetailResponse {
  ...
  @ApiProperty({ type: [SupervisorSummaryResponse] })
...
export class ActivityLogItem {
  ...
  @ApiProperty({ nullable: true, example: 'أحمد المعلم' }) actor_name!:
    | string
    | null;
  @ApiProperty({ nullable: true, example: null }) target_user_id!:
    | number
    | null;
  @ApiProperty({ nullable: true, example: null }) target_user_name!:
    | string
    | null;
  @ApiProperty({ nullable: true, example: 42 }) target_student_id!:
    | number
    | null;
  @ApiProperty({ nullable: true, example: 'محمد علي' }) target_student_name!:
    | string
    | null;
  @ApiProperty({ nullable: true, example: null }) from_halaqa_id!:
    | number
    | null;
