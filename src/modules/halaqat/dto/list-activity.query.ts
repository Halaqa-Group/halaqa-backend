...
export class ListActivityQuery {
  ...
  @ApiProperty({ required: false, enum: ACTIVITY_ACTIONS, example: 'student_enrolled' })
  @IsOptional()
  @IsEnum(ACTIVITY_ACTIONS)
  action?: HalaqaActivityAction;

  @ApiProperty({ required: false, format: 'date', example: '2026-05-01', description: 'Inclusive lower bound on created_at.' })
  @IsOptional()
  @IsDateString()
  from_date?: string;

  @ApiProperty({ required: false, format: 'date', example: '2026-05-31', description: 'Inclusive upper bound on created_at.' })
  @IsOptional()
  @IsDateString()
  to_date?: string;
}
