...
function makeConflictChecker(conflicts: unknown[] = []) {
  return {
    checkForTeacher: jest.fn().mockResolvedValue(conflicts),
  } as never;
}
...
function makeService(overrides: {
  ...
  return new TeacherAssignmentService(
    ...
    (overrides.conflictChecker ?? makeConflictChecker()) as ScheduleConflictService,
...
describe('TeacherAssignmentService', () => {
  ...
  describe('assign()', () => {
    it('verifies halaqa access and teacher role in school', async () => {
      ...
      await svc.assign(17, { teacher_user_id: 12, role: 'main', start_date: '2026-05-01' }, PRINCIPAL);
      expect(halaqatService.loadAndCheckAccess).toHaveBeenCalledWith(17, PRINCIPAL);
      expect(halaqatService.verifyUserRoleInSchool).toHaveBeenCalledWith(12, 10, 'teacher');
    });
    ...
    it('throws 409 when teacher already has active assignment', async () => {
      ...
      await expect(
        svc.assign(17, { teacher_user_id: 12, role: 'main', start_date: '2026-05-01' }, PRINCIPAL),
      ).rejects.toThrow(ConflictException);
    });

    it('throws 409 when schedule conflict detected', async () => {
      const conflictChecker = makeConflictChecker([
        { day_of_week: 0, prayer_slot: 'fajr', conflicting_halaqa_id: 5, conflicting_halaqa_name: 'أخرى' },
      ]);
      const svc = makeService({ conflictChecker });
      await expect(
        svc.assign(17, { teacher_user_id: 12, role: 'main', start_date: '2026-05-01' }, PRINCIPAL),
      ).rejects.toThrow(ConflictException);
    });
    ...
    it('saves assignment with correct fields', async () => {
      const repo = makeRepo();
      const svc = makeService({ repo });
      await svc.assign(17, { teacher_user_id: 12, role: 'assistant', start_date: '2026-05-01', notes: 'test' }, PRINCIPAL);
    ...
    it('returns mapped TeacherAssignmentResponse', async () => {
      ...
      const result = await svc.assign(17, { teacher_user_id: 12, role: 'main', start_date: '2026-05-01' }, PRINCIPAL);
      expect(result).toMatchObject({
        id: 55, teacher_user_id: 12, teacher_name: 'أحمد',
        role: 'main', acting_as_primary: false,
      });
    });
  });
  ...
  describe('findAll()', () => {
    ...
    it('returns active assignments mapped to response shape', async () => {
      const ds = makeDataSource([BASE_ROW, { ...BASE_ROW, id: 56, teacher_user_id: 13, teacher_name: 'محمد', role: 'assistant' }]);
  ...
  describe('updateAssignment()', () => {
    it('throws 404 when assignment not found', async () => {
      ...
      const svc = makeService({ repo });
      await expect(svc.updateAssignment(17, 55, { role: 'assistant' }, PRINCIPAL)).rejects.toThrow(NotFoundException);
    });
    ...
    it('throws 409 when assignment is already ended', async () => {
      ...
      const svc = makeService({ repo });
      await expect(svc.updateAssignment(17, 55, { role: 'assistant' }, PRINCIPAL)).rejects.toThrow(ConflictException);
    });

    it('throws 409 when trying to update a substitute assignment', async () => {
      const repo = makeRepo({
        findOne: jest.fn().mockResolvedValue({ ...BASE_ASSIGNMENT, role: 'substitute' }),
      });
      const svc = makeService({ repo });
      await expect(svc.updateAssignment(17, 55, { role: 'main' }, PRINCIPAL)).rejects.toThrow(ConflictException);
    });
    ...
    it('saves updated role and notes', async () => {
      ...
      const svc = makeService({ repo });
      await svc.updateAssignment(17, 55, { role: 'assistant', notes: 'stepped down' }, PRINCIPAL);
      expect(repo.save).toHaveBeenCalledWith(expect.objectContaining({ role: 'assistant', notes: 'stepped down' }));
    });

    it('logs teacher_role_changed only when role actually changes', async () => {
      const activityLog = makeActivityLog();
      const repo = makeRepo({ findOne: jest.fn().mockResolvedValue({ ...BASE_ASSIGNMENT, role: 'main' }) });
      const svc = makeService({ repo, activityLog });

      await svc.updateAssignment(17, 55, { role: 'assistant' }, PRINCIPAL);
      expect(activityLog.log).toHaveBeenCalledWith(expect.objectContaining({ action: 'teacher_role_changed' }));
    });
    ...
    it('does not log when role is unchanged', async () => {
      ...
      await svc.updateAssignment(17, 55, { role: 'main', notes: 'updated notes' }, PRINCIPAL);
  ...
  describe('endAssignment()', () => {
    ...
    it('throws 404 when assignment not found', async () => {
      const repo = makeRepo({ findOne: jest.fn().mockResolvedValue(null) });
      const svc = makeService({ repo });
      await expect(svc.endAssignment(17, 55, dto, PRINCIPAL)).rejects.toThrow(NotFoundException);
    });
    ...
    it('throws 409 when assignment is already ended', async () => {
      ...
      const svc = makeService({ repo });
      await expect(svc.endAssignment(17, 55, dto, PRINCIPAL)).rejects.toThrow(ConflictException);
    });

    it('throws 400 when end_date is before start_date', async () => {
      const repo = makeRepo({
        findOne: jest.fn().mockResolvedValue({ ...BASE_ASSIGNMENT, startDate: '2026-06-01' }),
      });
      const svc = makeService({ repo });
      await expect(
        svc.endAssignment(17, 55, { end_date: '2026-05-01', end_reason: 'vacation' }, PRINCIPAL),
