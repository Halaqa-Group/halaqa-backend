...
const BASE_ENROLLMENT: StudentHalaqa = {
  studentId: 42, halaqaId: 17,
  enrollmentDate: '2026-01-01', status: 'active',
  student: {} as never, halaqa: {} as never,
};

const BASE_ROW = {
  student_id: 42, student_name: 'محمد علي',
...
describe('StudentEnrollmentService', () => {
  ...
  describe('enroll()', () => {
    it('throws 409 when halaqa is not active', async () => {
      const svc = makeService({ halaqatService: makeHalaqatService(ARCHIVED_HALAQA) });
      await expect(
    ...
    it('throws 404 when student not found in school', async () => {
      ...
      (ds.manager.query as jest.Mock).mockImplementation(async (sql: string) => {
    ...
    it('throws 409 when student is already actively enrolled', async () => {
      const repo = makeRepo({ findOne: jest.fn().mockResolvedValue({ ...BASE_ENROLLMENT, status: 'active' }) });
    ...
    it('creates new enrollment with provided date', async () => {
      ...
      const svc = makeService({ repo });
      await svc.enroll(17, { student_id: 42, enrollment_date: '2026-06-01' }, PRINCIPAL);
      expect(repo.create).toHaveBeenCalledWith(expect.objectContaining({
        studentId: 42, halaqaId: 17,
        enrollmentDate: '2026-06-01', status: 'active',
      }));
    });
    ...
    it('defaults enrollment_date to today', async () => {
      ...
      expect(repo.create).toHaveBeenCalledWith(expect.objectContaining({ enrollmentDate: TODAY }));
    });
    ...
    it('logs student_enrolled for new enrollment', async () => {
      ...
      expect(activityLog.log).toHaveBeenCalledWith(expect.objectContaining({
        action: 'student_enrolled', halaqaId: 17, targetStudentId: 42,
      }));
    });
    ...
    it('logs student_re_enrolled when prior enrollment exists', async () => {
      const activityLog = makeActivityLog();
      const repo = makeRepo({
        findOne: jest.fn().mockResolvedValue({ ...BASE_ENROLLMENT, status: 'completed' }),
      });
      const svc = makeService({ repo, activityLog });
      await svc.enroll(17, { student_id: 42 }, PRINCIPAL);
      expect(activityLog.log).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('returns mapped StudentEnrollmentResponse', async () => {
      ...
      expect(result).toMatchObject({
        student_id: 42, student_name: 'محمد علي', status: 'active',
      });
    });
  });
  ...
  describe('listStudents()', () => {
    ...
    it('returns mapped list of active students', async () => {
      const rows = [
        BASE_ROW,
        { student_id: 43, student_name: 'أحمد', enrollment_date: '2026-02-01', status: 'active' },
      ];
      ...
      expect(result[0]).toMatchObject({ student_id: 42, student_name: 'محمد علي' });
    });
  ...
  describe('removeStudent()', () => {
    ...
    it('logs student_completed for completed outcome', async () => {
      const activityLog = makeActivityLog();
      const repo = makeRepo({ findOne: jest.fn().mockResolvedValue({ ...BASE_ENROLLMENT }) });
      const svc = makeService({ repo, activityLog });
      await svc.removeStudent(17, 42, { outcome: 'completed' }, PRINCIPAL);
      expect(activityLog.log).toHaveBeenCalledWith(expect.objectContaining({
        action: 'student_completed', targetStudentId: 42,
      }));
    });
    ...
    it('logs student_unenrolled for unenrolled outcome', async () => {
      ...
      const repo = makeRepo({ findOne: jest.fn().mockResolvedValue({ ...BASE_ENROLLMENT }) });
      const svc = makeService({ repo, activityLog });
      await svc.removeStudent(17, 42, { outcome: 'unenrolled', notes: 'dropped out' }, PRINCIPAL);
      expect(activityLog.log).toHaveBeenCalledWith(expect.objectContaining({
        action: 'student_unenrolled', notes: 'dropped out',
      }));
    });
    ...
    it('returns ApiMessage', async () => {
      const repo = makeRepo({ findOne: jest.fn().mockResolvedValue({ ...BASE_ENROLLMENT }) });
      const svc = makeService({ repo });
      const result = await svc.removeStudent(17, 42, { outcome: 'completed' }, PRINCIPAL);
  ...
  describe('transferStudent()', () => {
    ...
    it('throws 409 when source halaqa is not active', async () => {
      ...
      const svc = makeService({ halaqatService });
      await expect(svc.transferStudent(dto, PRINCIPAL)).rejects.toThrow(ConflictException);
    });
    ...
    it('throws 404 when student not actively enrolled in source', async () => {
      ...
      await expect(svc.transferStudent(dto, PRINCIPAL)).rejects.toThrow(NotFoundException);
    });
    ...
    it('marks source enrollment as transferred and creates destination enrollment', async () => {
      ...
      expect(saveCalls[1][0]).toMatchObject({ halaqaId: 19, studentId: 42, status: 'active' });
    });
    ...
    it('uses transfer_date from dto when provided', async () => {
      ...
      const halaqatService = makeHalaqatService();
      (halaqatService.loadAndCheckAccess as jest.Mock)
        .mockResolvedValue(ACTIVE_HALAQA);
      const svc = makeService({ repo, halaqatService });
      await svc.transferStudent({ ...dto, transfer_date: '2026-06-01' }, PRINCIPAL);
      const saveCalls = (repo.save as jest.Mock).mock.calls;
    ...
    it('defaults transfer_date to today', async () => {
      ...
      const halaqatService = makeHalaqatService();
      (halaqatService.loadAndCheckAccess as jest.Mock).mockResolvedValue(ACTIVE_HALAQA);
      const svc = makeService({ repo, halaqatService });
      await svc.transferStudent(dto, PRINCIPAL);
      const saveCalls = (repo.save as jest.Mock).mock.calls;
    ...
    it('logs student_transferred_out and student_transferred_in', async () => {
      ...
      expect(activityLog.log).toHaveBeenCalledWith(expect.objectContaining({
        action: 'student_transferred_out', halaqaId: 17, targetStudentId: 42,
      }));
      expect(activityLog.log).toHaveBeenCalledWith(expect.objectContaining({
        action: 'student_transferred_in', halaqaId: 19, targetStudentId: 42,
      }));
    });
