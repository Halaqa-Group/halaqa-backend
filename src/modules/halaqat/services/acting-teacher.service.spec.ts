...
describe('ActingTeacherService', () => {
  ...
  describe('setActing()', () => {
    it('throws 404 when assignment not found', async () => {
      const svc = makeService({
        repo: makeRepo({ findOne: jest.fn().mockResolvedValue(null) }),
      });
      await expect(
        svc.setActing(
          17,
          55,
          { acting_starts_at: TODAY, acting_ends_at: '2026-05-20' },
          PRINCIPAL,
        ),
    ...
    it('throws 409 when teacher is already acting as primary', async () => {
      const repo = makeRepo({
        findOne: jest
          .fn()
          .mockResolvedValue({ ...BASE_ASSIGNMENT, actingAsPrimary: true }),
      });
      const svc = makeService({ repo });
      await expect(
        svc.setActing(
    ...
    it('throws 400 when acting_starts_at is in the past', async () => {
      ...
      await expect(
        svc.setActing(
          17,
          55,
          { acting_starts_at: '2026-05-01', acting_ends_at: '2026-05-20' },
          PRINCIPAL,
        ),
    ...
    it('sets actingAsPrimary=true immediately when acting_starts_at is today', async () => {
      ...
      expect(repo.save).toHaveBeenCalledWith(
        expect.objectContaining({ actingAsPrimary: true }),
      );
    });
    ...
    it('leaves actingAsPrimary=false when acting_starts_at is in the future', async () => {
      ...
      await svc.setActing(
        17,
        55,
        { acting_starts_at: '2026-05-15', acting_ends_at: '2026-05-25' },
        PRINCIPAL,
      );
      expect(repo.save).toHaveBeenCalledWith(
        expect.objectContaining({ actingAsPrimary: false }),
      );
    });
    ...
    it('returns mapped TeacherAssignmentResponse', async () => {
      ...
      const result = await svc.setActing(
        17,
        55,
        { acting_starts_at: TODAY, acting_ends_at: '2026-05-20' },
        PRINCIPAL,
  ...
  describe('substitute()', () => {
    ...
    it('throws 400 when acting_starts_at is in the future', async () => {
      ...
      await expect(
        svc.substitute(
          17,
          { ...dto, acting_starts_at: '2026-05-15' },
          PRINCIPAL,
        ),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('throws 409 when teacher already has active assignment in halaqa', async () => {
      ...
      const svc = makeService({ repo });
      await expect(svc.substitute(17, dto, PRINCIPAL)).rejects.toThrow(
        ConflictException,
      );
    });
  ...
  describe('extend()', () => {
    it('throws 404 when no active acting teacher', async () => {
      ...
      const svc = makeService({ repo });
      await expect(
        svc.extend(17, { acting_ends_at: '2026-06-01' }, PRINCIPAL),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('saves updated acting_ends_at', async () => {
      ...
      expect(repo.save).toHaveBeenCalledWith(
        expect.objectContaining({ actingEndsAt: '2026-06-01' }),
      );
    });
  ...
  describe('endActing()', () => {
    it('throws 404 when no active acting teacher', async () => {
      const repo = makeRepo({ findOne: jest.fn().mockResolvedValue(null) });
      const svc = makeService({ repo });
      await expect(svc.endActing(17, PRINCIPAL)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('ends the assignment row when teacher is substitute', async () => {
      const repo = makeRepo({
        findOne: jest
          .fn()
          .mockResolvedValue({ ...ACTING_ASSIGNMENT, role: 'substitute' }),
        save: jest.fn().mockImplementation(async (e) => e),
      });
      const svc = makeService({ repo });
      await svc.endActing(17, PRINCIPAL);
    ...
    it('does not set endDate for non-substitute acting teacher', async () => {
      const repo = makeRepo({
        findOne: jest
          .fn()
          .mockResolvedValue({ ...ACTING_ASSIGNMENT, role: 'main' }),
        save: jest.fn().mockImplementation(async (e) => e),
      });
      const svc = makeService({ repo });
      await svc.endActing(17, PRINCIPAL);
